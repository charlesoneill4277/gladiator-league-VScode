...
interface EditableTeamCellProps {
  ...
  teamPosition: 'team1' | 'team2';
  isWinner: boolean;
  allTeams: TeamWithDetails[];
  onTeamChange: (matchupId: number, teamPosition: 'team1' | 'team2', newTeamId: number) => void;
}
...
const EditableTeamCell: React.FC<EditableTeamCellProps> = ({
  ...
  const handleSaveTeamChange = () => {
    ...
    if (newTeamId && newTeamId !== team?.id) {
      onTeamChange(matchupId, teamPosition, newTeamId);
    }
    setIsEditing(false);
  };
...
const RegularSeasonMatchups: React.FC = () => {
  ...
  const loadCurrentWeek = async () => {
    try {
      ...
      // Only set week if it's regular season (1-12)
      if (currentWeek <= 12) {
        setSelectedWeek(currentWeek.toString());
      } else {
        setSelectedWeek('1'); // Default to week 1 if current week is playoffs
      }
    } catch (error) {
      console.error('Error loading current week:', error);
      setSelectedWeek('1'); // Default to week 1
    }
  };
  ...
  const loadMatchups = async () => {
    setLoading(true);
    console.log(`Loading regular season matchups for season ${selectedSeason}, week ${selectedWeek}`);
    ...
    try {
      ...
      // Fetch matchups for all conferences (regular season only - weeks 1-12)
      const allMatchups: MatchupWithConference[] = [];

      for (const conferenceId of conferenceIds) {
        console.log(`Fetching regular season matchups for conference ${conferenceId}`);
        ...
        const { data, error } = await DatabaseService.getMatchups({
          filters: [
            ...
            { column: 'is_playoff', operator: 'eq', value: false } // Only regular season
          ],
        ...
        const matchups = data || [];
        console.log(`Found ${matchups.length} regular season matchups for conference ${conferenceId}`);

        const conference = conferences.find((c) => c.id === conferenceId);
        const matchupsWithConference = matchups.map((matchup: any) => {
          console.log(`Loaded regular season matchup ${matchup.id}:`, {
      ...
      console.log(`Total regular season matchups loaded: ${allMatchups.length}`);
      console.log('Regular season matchups breakdown by status:', {
      ...
      console.error('Error loading regular season matchups:', error);
      toast({
        title: 'Error',
        description: `Failed to load regular season matchups: ${error}`,
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
 ...
 const handleDragStart = (event: any) => {
  ...
  const handleTeamChange = async (matchupId: number, teamPosition: 'team1' | 'team2', newTeamId: number) => {
    console.log(`Changing team for regular season matchup ${matchupId}, position ${teamPosition} to team ${newTeamId}`);

    // First update the matchup with the new team
    setMatchups((prev) => prev.map((matchup) => {
      if (matchup.id === matchupId) {
        ...
        if (teamPosition === 'team1') {
          updatedMatchup.team1_id = newTeamId;
        } else {
          updatedMatchup.team2_id = newTeamId;
        }

        console.log(`Updated regular season matchup ${matchupId}: team1_id=${updatedMatchup.team1_id}, team2_id=${updatedMatchup.team2_id}`);
    ...
    // Then fetch and update scores for the modified matchup
    const matchup = matchups.find(m => m.id === matchupId);
    if (matchup) {
      try {
        // Get the updated team IDs
        const team1Id = teamPosition === 'team1' ? newTeamId : matchup.team1_id;
        const team2Id = teamPosition === 'team2' ? newTeamId : matchup.team2_id;

        console.log(`Fetching regular season scores for updated matchup ${matchupId}: team1=${team1Id}, team2=${team2Id}, week=${matchup.week}`);

        // Fetch current scores from Sleeper
        const { team1Score, team2Score } = await fetchSleeperScoresForTeams(
          team1Id,
          team2Id,
          matchup.conference_id,
          matchup.week
        );

        // Update the matchup with the fetched scores
        setMatchups((prev) => prev.map((m) => {
          if (m.id === matchupId) {
            ...
            console.log(`🏆 REGULAR SEASON TEAM CHANGE WINNER - Matchup ${matchupId}: Team1(${team1Id})=${team1Score}, Team2(${team2Id})=${team2Score}, Winner=${winningTeamId}`);
        ...
        console.log(`✅ Updated regular season matchup ${matchupId} with scores: ${team1Score} - ${team2Score}`);

      } catch (error) {
        console.error(`Error fetching regular season scores for matchup ${matchupId}:`, error);
    ...
    toast({
      title: 'Team Changed',
      description: `Regular season matchup ${matchupId} ${teamPosition} changed to ${newTeam?.team_name}. Scores updated automatically. Remember to save changes.`,
      duration: 3000
    });
  };
  ...
  const handleSaveChanges = async () => {
    setSaving(true);
    console.log('Starting save operation for', matchups.length, 'regular season matchups');
    ...
    try {
      ...
      for (const matchup of matchups) {
        console.log(`Updating regular season matchup ${matchup.id}:`, {
        ...
        const updateData = {
          ...
          is_playoff: false, // Regular season matchups
        ...
        console.log(`Sending update for regular season matchup ${matchup.id} with data:`, updateData);

        const updateResult = await DatabaseService.updateMatchup(matchup.id, updateData);

        if (updateResult.error) {
          console.error(`❌ Failed to update regular season matchup ${matchup.id}:`, updateResult.error);
          ...
          console.log(`✅ Successfully updated regular season matchup ${matchup.id}`);
      ...
      if (failureCount > 0) {
        ...
        toast({
          ...
          description: `${successCount} regular season matchups updated successfully, ${failureCount} failed. Check console for details.`,
          variant: 'destructive'
        });
      } else {
        setHasChanges(false);
        toast({
          title: 'Success',
          description: `All ${successCount} regular season matchups updated successfully`
        });
      }

      // Reload matchups to ensure we have the latest data
      console.log('Reloading regular season matchups to verify changes persisted...');
      await loadMatchups();
      ...
      toast({
        title: 'Error',
        description: `Failed to save regular season matchup changes: ${error}`,
        variant: 'destructive'
      });
    } finally {
      setSaving(false);
  ...
  // Regular season weeks only (1-12)
  const regularSeasonWeeks = Array.from({ length: 12 }, (_, i) => i + 1);
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Week (Regular Season)</label>
              <Select
                value={selectedWeek}
      ...
      <Card>
          <CardContent className="flex items-center justify-center py-12">
            <div className="flex items-center gap-2">
              ...
              <span>Loading regular season matchups...</span>
            </div>
          </CardContent>
        </Card> :
      ...
      <Card>
          <CardContent className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No Regular Season Matchups Found</h3>
              <p className="text-gray-600">
                No regular season matchups found for the selected week. They may need to be created first.
              </p>
            </div>
          </CardContent>
        </Card> :
      ...
      <Card>
          <CardContent className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Please select a season and week to manage regular season matchups across all conferences.
              </p>
            </div>
          </CardContent>
        </Card>
...
export default RegularSeasonMatchups;