...
export async function testDataIntegrityFixes() {
  ...
  try {
    ...
    try {
      const standings2024 = await teamRecordsService.getStandingsData(1); // Assuming season ID 1 is 2024
    ...
    try {
      const standings2025 = await teamRecordsService.getStandingsData(2); // Assuming season ID 2 is 2025
