...
const TransactionsPage = () => {
  ...
  const { 
    ...
    limit: 100, // Get more transactions for the full page
    conferenceIds,
    teamFilter: filters.team,
    typeFilter: filters.type,
    autoRefresh: false // Don't auto-refresh on this page
  });
  ...
  useEffect(() => {
    ...
    if (filters.conference !== 'all') {
      filtered = filtered.filter(transaction => 
        transaction.teams.some(team => team.includes(filters.conference))
      );
    }
    ...
    if (filters.player.trim()) {
      ...
      filtered = filtered.filter(transaction => 
        transaction.players.added.some(player => player.name.toLowerCase().includes(playerSearch)) ||
        transaction.players.dropped.some(player => player.name.toLowerCase().includes(playerSearch))
      );
    }
